...
export async function installCamp127Data({root='dist',output=root,modelRoot='dist'}={}){
 ...
 const model=JSON.parse(await fs.readFile(path.join(modelRoot,'data/camp127/index.json'))),ids=new Set(model.excludeIds),hashes=new Set([...ids].map(facadeHash));
 ...
 fac.protectedIds=[...new Set([...fac.protectedIds,...ids])].sort();fac.stats.texturedBuildings-=removed.length;fac.stats.facades-=removedRecords;fac.stats.geometryBytes-=removedRecords*52;fac.stats.excludedCamp127=(fac.stats.excludedCamp127||0)+removed.length;
 for(const r of removed){fac.stats.sectorCounts[r.sector]--;for(const f of r.faces)fac.stats.materialCounts[f[3]]--;}
 assign.buildings=assign.buildings.filter(b=>!ids.has(b.id));for(const id of ids)if(!assign.excluded.some(e=>e.id===id))assign.excluded.push({id,reason:'camp127'});
 ...
 const preserved=[],old=[];for(const node of walk.nodes){const name=path.basename(node[0],'.bin').replace('-','/');(node[0].startsWith('camp127/')||ids.has(name)?old:preserved).push(node);}
 ...
 const bytes=await fs.readFile(path.join(modelRoot,'data/camp127/mesh.bin')),a=new Float32Array(bytes.buffer,bytes.byteOffset,bytes.byteLength/4),packs=new Map();
 for(const range of model.ranges)for(let first=range.first;first<range.first+range.count;first+=3){const points=Array.from({length:3},(_,k)=>Array.from(a.subarray((first+k)*11,(first+k+1)*11)));for(const p of points){const geo=[model.origin[0]+p[0]/model.scale[0],model.origin[1]+p[1]/model.scale[1]];p[0]=(geo[0]-walk.origin[0])*walk.scale[0];p[1]=(geo[1]-walk.origin[1])*walk.scale[1];}
  const x=points.reduce((s,p)=>s+p[0],0)/3,y=points.reduce((s,p)=>s+p[1],0)/3,key=`camp127-${Math.floor(x/20)}-${Math.floor(y/20)}`;
 ...
 walk.sourceHashes['dist/data/camp127/index.json']=sha(await fs.readFile(path.join(modelRoot,'data/camp127/index.json')));walk.sourceHashes['dist/data/camp127/mesh.bin']=sha(bytes);
 ...
 const retired=new Set(walk.retiredNodes||[]),emptyHeader=Buffer.from(JSON.stringify({ranges:[],segments:[],replacedBy:'camp127'}));
 ...
 for(const [file]of old)if(file.startsWith('camp127/')&&!nodes.some(n=>n[0]===file)){await put('data/walk/'+file,empty);retired.add(file);}
 // Keep metadata for already empty legacy replacements from older models.
 const previousRegistry=await json('data/custom-models.json');
 for(const id of new Set(previousRegistry.models.flatMap(m=>m.excludeIds))){
  const match=/^(way|relation)\/(\d+)$/.exec(id);if(!match)continue;
  const file=String(Number(match[2])%100)+'/'+id.replace('/','-')+'.bin';
  if(nodes.some(n=>n[0]===file))continue;
  try{const bytes=await read('data/walk/'+file),size=bytes.readUInt32LE(0);if(bytes.length!==size+4)continue;const header=JSON.parse(bytes.toString('utf8',4,size+4));if(header.ranges?.length===0&&header.segments?.length===0)retired.add(file);}catch(e){if(e.code!=='ENOENT')throw e;}
 }
 walk.retiredNodes=[...retired].sort();
 await put('data/walk/index.json',walk);
 const registry={version:1,loadRadiusMeters:500,models:[]};
 for(const dir of await fs.readdir(path.join(root,'data'),{withFileTypes:true})){if(!dir.isDirectory()||dir.name==='camp127')continue;let index;try{index=await json('data/'+dir.name+'/index.json');}catch(e){if(e.code==='ENOENT')continue;throw e;}if(index.vertexCount&&index.ranges&&index.excludeIds&&index.bounds&&index.origin&&index.materials&&index.textures)registry.models.push({id:dir.name,layerId:dir.name+'-detail',basePath:'./data/'+dir.name,label:index.name||dir.name,bounds:index.bounds,excludeIds:index.excludeIds});}
 registry.models.push({id:'camp127',layerId:'camp127-detail',basePath:'./data/camp127',label:model.name,bounds:model.bounds,excludeIds:model.excludeIds});registry.models.sort((a,b)=>a.id.localeCompare(b.id));await put('data/custom-models.json',registry);
 // Keep compressed copies consistent when the local game is rebuilt later.
 for(const file of new Set(['data/camp127/index.json','data/camp127/mesh.bin','data/custom-models.json',...changed.map(c=>c.file)])){
  let bytes;try{bytes=await fs.readFile(path.join(output,file));}catch(e){if(e.code==='ENOENT')continue;throw e;}
  await fs.writeFile(path.join(output,file+'.gz'),gzipSync(bytes,{level:9}));
 }
 return {changed,removedFacadeRecords:removedRecords,removedRoofTriangles:removedTriangles,preservedWalkNodes:preserved.length,camp127Nodes:packs.size};
}
if(process.argv[1]&&import.meta.url===pathToFileURL(path.resolve(process.argv[1])).href){const report=await installCamp127Data();await fs.mkdir('artifacts/camp127',{recursive:true});await fs.writeFile('artifacts/camp127/installation.json',JSON.stringify(report,null,2));console.log(JSON.stringify({...report,changed:report.changed.length}));}
