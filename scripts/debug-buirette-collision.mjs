...
assert.equal(index.parts.length,5);assert.equal(index.stats.mainBuildings,3);assert.equal(raw.length,index.vertexCount*44);assert.equal(survey.openings.length,25);
assert.deepEqual(index.parts.filter(p=>p.address).map(p=>p.address),['12','14','16']);assert(index.parts.filter(p=>!p.address).every(p=>p.confidence.includes('non confirmée')));
...
for(const r of survey.references){assert.equal(r.image_date,'2024-07');assert.equal(sha(await read('data/buirette/'+r.file)),r.sha256);}
...
for(const p of index.parts.filter(p=>p.address)){const front=p.front.map(toLocal),mid=front[0].map((a,i)=>(a+front[1][i])/2),n=p.frame.v.map(x=>-x),start=mid.map((x,i)=>x+n[i]*1),end=stepPlayer(start,-n[0]*2,-n[1]*2,segments);console.log({wall:p.id,start,end,distance:Math.hypot(...end.map((x,i)=>x-start[i])),normal:-(end[0]-start[0])*n[0]-(end[1]-start[1])*n[1]});}
