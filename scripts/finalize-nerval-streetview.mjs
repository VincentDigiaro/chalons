...
const root=(await fs.readFile('artifacts/nerval-streetview-current.txt','utf8')).trim();
...
const files=['dist/data/nerval/index.json','dist/data/nerval/mesh.bin','dist/data/nerval/survey.json','dist/data/walk/index.json',download,...walk.nodes.filter(n=>n[0].startsWith('detail/')).map(n=>'dist/data/walk/'+n[0])];
...
const inScope=p=>/^dist\/data\/(nerval\/(index\.json|survey\.json|mesh\.bin|buildings\.geojson)|walk\/(index\.json|detail\/[^/]+)|walk-downloads\/index\.json)(\.gz)?$/.test(p)||['scripts/build-nerval.mjs','scripts/build-walk.mjs','scripts/README-nerval.md'].includes(p);
...
for(const file of ['scripts/build-nerval-streetview.mjs','scripts/nerval-streetview-survey.json','scripts/check-nerval-streetview.mjs','scripts/finalize-nerval-streetview.mjs']){const b=await fs.readFile(file);changes.push({path:file,before:null,after:sha(b),bytes:b.length});}
