...
// Protect the actual custom buildings, not their neighbourhood's bounding box.
// Plain OSM neighbours use the same deterministic facade catalogue as the city.
const excludedIds=new Set(await detailedBuildingIds());

const camp127Ids=new Set(JSON.parse(await fs.readFile('dist/data/camp127/index.json','utf8')).excludeIds);
const parc14Ids=new Set(JSON.parse(await fs.readFile('dist/data/parc14/index.json','utf8')).excludeIds);
const buiretteIds=new Set(JSON.parse(await fs.readFile('dist/data/buirette/index.json','utf8')).excludeIds);
...
const stats={sourceBuildings:source.features.length,texturedBuildings:0,facades:0,excludedNerval:0,excludedAttila:0,excludedBuirette:0,excludedParc14:0,excludedCamp127:0,excludedExisting:0,excludedNonWalls:0,shortEdges:0,materialCounts:Array(catalog.materials.length).fill(0),sectorCounts:{}};
...
function chooseMaterial(p,center,area,sector,seed){
  const inActivity=activity.some(a=>intersects(a.bounds,[...center,...center])&&inPolygon(center,a.rings));
  return chooseFacade(p,center,area,sector,seed,inActivity);
}
...
for(const f of [...source.features].sort((a,b)=>a.properties.osm_id.localeCompare(b.properties.osm_id))){
  ...
  if(excludedIds.has(id)){const reason=detailed.excludeIds.includes(id)?'nerval':buiretteIds.has(id)?'buirette':parc14Ids.has(id)?'parc14':camp127Ids.has(id)?'camp127':'attila';stats[{nerval:'excludedNerval',attila:'excludedAttila',buirette:'excludedBuirette',parc14:'excludedParc14',camp127:'excludedCamp127'}[reason]]++;excluded.push({id,reason});continue;}
  ...
  for(let pi=0;pi<polys.length;pi++)for(let ri=0;ri<polys[pi].length;ri++){
    ...
    for(let i=0;i<points.length;i++){
      ...
      const height=p.height-p.min_height,mat=wallMaterial(material,catalog.materials[material],length,longest,height,hash(`${id}/${pi}/${ri}/${i}`));
      const [repeatX,floors]=facadeRepeats(catalog.materials[mat],length,height);
...
const index={version:1,zoom:ZOOM,stride:52,vertexFormat:'12 float32-le: ax,ay,bx,by,base_m,top_m,repeat_u,repeat_v,material,light,tint,reserved; uint32-le: building_hash',catalogue:'catalogue.json',textureSize:[512,256],textures:catalog.materials.map(m=>`textures/${m.id}.webp`),protectedIds:[...excludedIds].sort(),sourceSha256:crypto.createHash('sha256').update(sourceBytes).digest('hex'),stats:{...stats,chunks:chunkIndex.length,geometryBytes:chunkIndex.reduce((a,c)=>a+c.bytes,0)},chunks:chunkIndex};
...
await fs.writeFile('artifacts/facades/preservation.json',JSON.stringify({protectedHashes,sourceSha256:index.sourceSha256,protectedIds:index.protectedIds},null,2));
