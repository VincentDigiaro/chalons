import {CITY,cityDataURL,cityHighwindConfig} from './city-config.js';
...
import {preserveMapCamera} from './map-view.js';
import {shipEnabled} from './fps-highwind.js';
import {readMapHighwind,saveMapHighwind} from './highwind-session.js';
import {preserveScorchesForMap} from './bomb-scorches.js';
import {preserveBombStockForMap} from './bomb-stock.js';
...
export class WalkMode {
 ...
 async start({entry=null,pilot=shipEnabled(globalThis.location?.search||'')&&!!SHIP_CONFIG?.present}={}){
  if(this.phase!=='map')return;
  const directEntry=!!entry||!this.map,events=this.events=new AbortController(),current=()=>this.events===events&&!events.signal.aborted;
  this.approachRestore=null;this.streetName=undefined;this.streets=null;
  this.position=[...SPAWN];this.yaw=SPAWN_YAW;this.pitch=0;
  const savedShip=readMapHighwind();
  if(pilot){const start=savedShip??cityHighwindConfig(SHIP_CONFIG);this.position=[start.position.x,start.position.y];this.yaw=start.angleDegres*Math.PI/180;}
  else if(entry){this.position=[...entry.position];this.yaw=entry.yaw;this.pitch=entry.pitch;}
  else if(CITY.id==='chalons'){const center=this.map.getCenter();if(Math.hypot((center.lng-4.3728)*73000,(center.lat-48.9651)*111320)<200){this.position=toLocal([4.372708,48.9651469]);this.yaw=335*Math.PI/180;}}
  this.phase='loading';this.onStart();this.loadingButton(true);
  this.root=document.createElement('section');this.root.id='walk-root';this.root.setAttribute('aria-label','Promenade dans '+CITY.label);
  ...
  try{
   ...
   if(savedShip&&this.renderer.highwind.enabled){Object.assign(this.renderer.highwind.pose,savedShip);this.renderer.highwind.terrainPlaced=true;}
   this.flight=new HighwindFlight(this.renderer.highwind);this.highwindUI=new HighwindUI(this);this.music=SHIP_ID?(preloadHighwindMusic()||new HighwindMusic({shipId:SHIP_ID})):null;this.piss=new WalkPissAction(this);
   ...
   if(!current())return;
   if(pilot){
    const ship=this.renderer.highwind;await ship.initializing;await ship.residency?.pending;
    if(!current())return;if(ship.error||!ship.residency?.data?.gpu)throw Error(ship.error||ship.label+' indisponible');
    while(current()&&!ship.textureKeys().every(key=>this.renderer.textures.get(key)?.gpu)){if(ship.textureKeys().some(key=>this.renderer.textures.get(key)?.failed))throw Error('Une texture du vaisseau n’a pas pu charger');await new Promise(r=>setTimeout(r,20));}
    if(!current())return;this.flight.mode='flying';this.flight.cameraYaw=ship.pose.angleDegres*Math.PI/180;this.feet=ship.pose.position.z;this.grounded=false;this.verticalSpeed=0;this.highwindMusicArmed=true;this.music?.start();
   }else{this.feet=this.renderer.groundHeight(this.position);this.verticalSpeed=0;this.grounded=true;Object.assign(this,advancePlayer(this,0,0,.016,this.renderer.collisionScene(this.position)));}
   this.loadingButton(false);this.playButton.classList.add('is-entering');
   const targetCamera=pilot?this.flight.camera():{position:this.position,height:EYE_HEIGHT+this.feet,yaw:this.yaw,pitch:this.pitch};
   if(entry||!this.map){
    ...
    this.arrivalCamera={position:toLngLat(targetCamera.position),height:targetCamera.height,pitch:90+targetCamera.pitch*180/Math.PI,fov:FPS_FOV,direct:true};
   }else{
   ...
   this.approachOrigin=preserveMapCamera(this.map,FPS_FOV);
   ...
   const approach=this.map.calculateCameraOptionsFromCameraLngLatAltRotation(toLngLat(targetCamera.position),targetCamera.height,targetCamera.yaw*180/Math.PI,pilot?90+targetCamera.pitch*180/Math.PI:84,0);
   ...
   if(!pilot)this.pitch=(this.arrivalCamera.pitch-90)*Math.PI/180;
   }
   // Draw the first FPS frame at the same pose before exposing its canvas.
   const firstCamera=pilot?this.flight.camera():{position:this.position,height:EYE_HEIGHT+this.feet,yaw:this.yaw,pitch:this.pitch};this.renderer.draw(firstCamera.position,firstCamera.height,firstCamera.yaw,firstCamera.pitch,null,this.position);
   this.root.querySelector('#walk-message').hidden=true;
   ...
   if(location.hash==='#overview'){const url=new URL(location.href);url.hash='';history.replaceState(history.state,'',url.href);}
   this.canvas.focus({preventScroll:true});
   if(!directEntry&&navigator.userActivation?.isActive)this.lock();
 ...
 async loadStreets(){
  ...
  try{
   const response=await fetch(new URL(cityDataURL('lines.geojson'),import.meta.url),{signal:events.signal});
 ...
 bind(){
  ...
  // Direct flight can begin before a gesture; resume blocked audio on first input.
  const resumeFlightAudio=()=>{if(this.phase==='playing'&&this.flight?.active&&this.music?.wanted&&this.music.context?.state==='suspended')this.music.resume();};
  on(document,'pointerdown',resumeFlightAudio,{capture:true});on(document,'keydown',resumeFlightAudio,{capture:true});
  ...
  this.jumpInputs=new Set();this.nextJumpAt=0;this.bombInputs=new Set();
  on(document,'keydown',e=>{if(e.code==='Escape'){if(this.phase==='playing')this.pause();return;}if(e.code==='KeyB'&&this.phase==='playing'&&this.flight?.active){e.preventDefault();if(!e.repeat)this.holdBomb('keyboard');return;}if(e.code==='KeyP'&&this.phase==='playing'){e.preventDefault();if(!e.repeat)this.piss?.start();return;}if(e.code==='KeyE'&&this.phase==='playing'){e.preventDefault();if(!e.repeat)this.interact();return;}if(e.code==='Space'&&this.phase==='playing'){e.preventDefault();if(this.flight?.active)this.keys.add(e.code);else if(!e.repeat)this.holdJump('keyboard');return;}if(movementKeys.has(e.code)&&this.phase==='playing'){e.preventDefault();this.keys.add(e.code);}});
  ...
  on(document,'keyup',e=>{this.keys.delete(e.code);if(e.code==='Space')this.releaseJump('keyboard');if(e.code==='KeyB')this.releaseBomb('keyboard');});on(window,'blur',()=>this.pause());on(document,'visibilitychange',()=>{if(document.hidden)this.pause();});
  ...
  let resumingWithLeft=false;
  const cancelOrbit=()=>{this.mouseOrbit=false;this.mouseOrbitButtons=0;this.releaseBomb('mouse-left');resumingWithLeft=false;};
  const orbitButton=(e,down)=>{
   if(e.button!==0&&e.button!==2)return;
   const bit=e.button===0?1:2,previous=this.mouseOrbitButtons||0,orca=this.flight?.ship.shipId==='orca',playing=this.phase==='playing'&&this.flight?.active;
   if(orca&&bit===1){
    if(down){if(playing&&!resumingWithLeft&&e.pointerType!=='touch'&&!e.sourceCapabilities?.firesTouchEvents)this.holdBomb('mouse-left');}
    else{this.releaseBomb('mouse-left');resumingWithLeft=false;}
    return;
   }
   if(!orca&&bit===2&&playing){if(down&&!(previous&bit))this.flight.resetCameraView();else if(!down&&(previous&bit))this.flight.rememberCamera();}
   this.mouseOrbitButtons=down?previous|bit:previous&~bit;this.mouseOrbit=!!this.mouseOrbitButtons;
   if(this.mouseOrbit&&this.flight)this.flight.resetMouseTurn();
  };
  ...
  for(const event of ['pointerup','pointercancel','lostpointercapture'])on(this.canvas,event,e=>{if(this.flightTouch.end(e))return;if(this.lookPointer?.id===e.pointerId)this.lookPointer=null;if(event==='pointerup')orbitButton(e,false);else{this.releaseBomb('mouse-left');if(document.pointerLockElement!==this.canvas)cancelOrbit();}});
  ...
  on(window,'pagehide',()=>{this.clearJump();this.clearBomb();cancelAnimationFrame(this.frame);this.events.abort();this.piss?.dispose();this.music?.dispose();this.renderer?.dispose();});
 }
 ...
 dropBomb(){if(this.phase!=='playing'||!this.flight?.active)return false;this.renderer?.bombAudio?.unlock();return this.renderer.bombs?.drop(this.renderer.highwind,this.bombVelocity||[0,0,0])||false;}
 holdBomb(source){if(this.phase!=='playing'||!this.flight?.active||this.bombInputs.has(source))return;this.bombInputs.add(source);this.dropBomb();}
 releaseBomb(source){this.bombInputs?.delete(source);}
 clearBomb(){this.bombInputs?.clear();}
 repeatBomb(){if(this.phase!=='playing'||!this.flight?.active||!this.bombInputs?.size)return;const bombs=this.renderer?.bombs,limit=bombs?.config.dureeRechargeSecondes===0?bombs.config.simulation.largagesParImageSansRecharge:1;for(let i=0;i<limit;i++)if(!bombs?.drop(this.renderer.highwind,this.bombVelocity||[0,0,0]))break;}
 interactHighwind(){this.updateHighwindContact();const action=this.flight?.interact(this);if(!action)return;this.clearJump();this.clearBomb();this.keys.clear();this.stick=[0,0];this.lookPointer=null;this.flightTouch?.reset();this.mouseOrbit=false;this.mouseOrbitButtons=0;this.highwindUI?.reset();if(action==='board'){this.highwindMusicArmed=true;this.music?.start();this.canvas.focus();this.lock();}this.highwindUI?.update();}
 ...
 pause(unlock=true){if(this.phase!=='playing')return;this.phase='paused';this.lookPointer=null;this.flightTouch?.reset();this.mouseOrbit=false;this.mouseOrbitButtons=0;if(this.flight){this.flight.resetMouseTurn();this.flight.resetMovement();}this.piss?.update();this.keys.clear();this.setFlash(false);this.clearJump();this.clearBomb();this.stick=[0,0];this.highwindUI?.reset();this.music?.pause();this.renderer?.bombAudio?.pause();if(unlock&&document.pointerLockElement===this.canvas)document.exitPointerLock();const box=this.root.querySelector('#walk-message');box.hidden=false;this.root.querySelector('#walk-message-text').textContent='Promenade en pause';this.root.querySelector('#walk-resume').hidden=false;this.root.querySelector('#walk-resume').focus();}
 resume(){if(this.phase!=='paused')return;this.lock();this.keys.clear();this.last=performance.now();this.phase='playing';this.piss?.update();this.music?.resume();this.renderer?.bombAudio?.resume();this.root.querySelector('#walk-message').hidden=true;this.canvas.focus();}
 tick(now){
  ...
  if(this.phase==='playing'&&this.flight?.active){
   const before={...this.renderer.highwind.pose.position};
   this.flight.tick(Math.max(0,Math.min(dt,.05)),flightInputs(this.keys,this.stick,this.flightStick,this.touch),this.renderer);
   this.bombVelocity=['x','y','z'].map(k=>dt>0?(this.renderer.highwind.pose.position[k]-before[k])/Math.min(dt,.05):0);
  ...
  if(this.phase==='playing'){this.renderer.highwind?.animate(dt);this.renderer.bombs?.updateAim?.(this.renderer.highwind,this.flight?.active);this.renderer.bombs?.tick(dt);this.repeatBomb();if(this.flight&&!this.flight.active)this.updateHighwindContact();}
  ...
  if(this.phase==='playing')this.renderer.bombAudio?.update?.(this.renderer.bombs?.bombs||[],camera);
  this.reducedBombMotion??=globalThis.matchMedia?.('(prefers-reduced-motion:reduce)').matches??false;
  const visual=this.renderer.bombs?.cameraEffect(camera,this.reducedBombMotion),view=visual?.camera||camera,flash=this.root.querySelector('#highwind-blast-flash');if(flash)flash.style.opacity=this.phase==='playing'?visual?.flash||0:0;
  this.renderer.draw(view.position,view.height,view.yaw,view.pitch,this.piss?.getVisualState(),this.position);this.frame=requestAnimationFrame(t=>this.tick(t));
 }
 exit(){if(this.renderer?.highwind?.enabled)saveMapHighwind(this.renderer.highwind.pose);this.keys.clear();this.setFlash(false);this.clearJump();this.clearBomb();this.piss?.dispose();this.music?.dispose();if(document.pointerLockElement===this.canvas)document.exitPointerLock();const url=clearWalkEntry(location.href);url.hash=walkReturnHash(this.position);history.replaceState(null,'',url.href);preserveScorchesForMap();preserveBombStockForMap();location.reload();}
 getState(){return {mode:this.phase,piss:this.piss?.getState(),flight:this.flight?.getState(),music:this.music?.getState(),streetName:this.streetName||null,flashEnabled:this.flash,turboSpeedKmh:Math.round(TURBO_SPEED*3.6),position:toLngLat(this.position),localPosition:[...this.position],arrivalCamera:this.arrivalCamera,approachOrigin:this.approachOrigin,eyeHeight:EYE_HEIGHT,feetHeight:this.feet,grounded:this.grounded,verticalSpeed:this.verticalSpeed,jumps:this.jumpCount||0,yaw:this.yaw,pitch:this.pitch,pointerLocked:document.pointerLockElement===this.canvas,mobile:this.touch,streaming:this.renderer?.getState()};}
