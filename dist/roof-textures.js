...
export class RoofTextures {
  ...
  refresh(){
    ...
    if(!this.enabled||!this.buildingsVisible||this.map.getPitch()<1||this.map.getZoom()<14){for(const job of this.queue)if(this.cache.get(job.key)===job)this.cache.delete(job.key);this.queue=[];clearTimeout(this.pumpTimer);this.emitState();return;}
  ...
  pump(){
    ...
    this.pumpTimer=setTimeout(()=>{
      ...
      fetch(imageryURL(item.z,item.x,item.y),{mode:'cors',signal:this.abort.signal}).then(async response=>{
        if(!response.ok)throw Error(`IGN HTTP ${response.status}`);
        if(!response.headers.get('content-type')?.includes('image/'))throw Error('Image IGN indisponible');
        return createImageBitmap(await response.blob(),{imageOrientation:'none',premultiplyAlpha:'none'});
      }).then(bitmap=>{
  ...
  emitState(){
    ...
    this.stats.ready=ready;this.onState({ready,loading,errors,active:this.enabled&&this.buildingsVisible&&this.map.getPitch()>=1&&this.map.getZoom()>=14});
  }
  texture(key){
  ...
  render(gl,options){
    this.stats.draws=0;
    if(!this.program||!this.enabled||!this.buildingsVisible||this.map.getPitch()<1||this.map.getZoom()<14)return;
