...
// Collision data is derived from the loaded mesh; render vertices and materials stay intact.
...
const floorAt=(position,scene,max)=>Math.max(GROUND_HEIGHT,...surfaceHeights(position,scene.surfaces).filter(z=>z<=max),...scene.segments.filter(s=>s[5]<=max&&segmentDistance(position,s)<PLAYER_RADIUS-.005).map(s=>s[5]));
export function advancePlayer(body,dx,dy,seconds,scene,jump=false,speedActive=false){
 ...
 for(let i=0;i<count;i++){
  ...
  if(feet>oldFeet){
   ...
   const floor=floorAt(position,scene,oldFeet+.005);
  ...
  position=stepPlayer(position,dx/count,dy/count,scene.segments,feet);
