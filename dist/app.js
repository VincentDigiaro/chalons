...
import {mapBuildingsVisible,overviewCamera,directHighwindEntry} from './map-view.js';
import {FPS_CONFIG,SHIP_CONFIG} from './walk-config.js';
...
import {MapHighwind} from './map-highwind.js';
import {MapBombScorches} from './bomb-scorches.js';
import {MapBombFragments} from './bomb-map-fragments.js';
import {readMapHighwind} from './highwind-session.js';
import {installCityUI} from './city-ui.js';
...
const NAV_BOUNDS = CITY.navigationBounds;
const walkReturn=readWalkReturn(location.hash,NAV_BOUNDS);
const walkEntry=readWalkEntry(location.search,NAV_BOUNDS);
const autoPilot=directHighwindEntry(location.search,location.hash,SHIP_CONFIG);
...
let nervalLayer,nervalUI,attilaLayer,cityRoads,mapHighwind,bombFragments;
const customLayers=new Map();let customModels=[];
let walking=false,walkMode;
let destinations=CITY.destinations.map(d=>({...d}));
...
function addLayers(){
  const source=(name,extra={})=>map.addSource(name,{type:'geojson',data:cityDataURL(`${name}.geojson`),maxzoom:16,tolerance:name==='buildings'?0.2:0.6,...extra});
  source('land');source('lines',bombMapRoadData.lines?{data:bombMapRoadData.lines}:{});source('buildings',{promoteId:'osm_id'});source('places');
  ...
  map.addSource('ign-photo',{type:'raster',tiles:[IMAGERY_TILES],tileSize:256,maxzoom:19,bounds:CITY.navigationBounds.flat(),attribution:'Photographies aériennes © <a href="https://cartes.gouv.fr/" target="_blank" rel="noopener">IGN</a>'});
  map.addLayer({id:'ign-ground',type:'raster',source:'ign-photo',paint:{'raster-fade-duration':200,'raster-resampling':'linear'}});
  map.addSource('city-roads-overview',{type:'geojson',data:bombMapRoadData.overview||cityDataURL('city-roads/overview.geojson'),maxzoom:14,tolerance:.4});
  ...
  cityRoads=new CityRoadsLayer();map.addLayer(cityRoads);map.addLayer(new MapBombScorches());
  ...
  bombFragments=new MapBombFragments();map.addLayer(bombFragments);
  if(CITY.id==='chalons')nervalUI=new NervalUI(map);
  installCustomModels();
...
async function installCustomModels(){
 try{
  const response=await fetch(cityDataURL('custom-models.json'));if(!response.ok)throw Error('Custom model registry HTTP '+response.status);
  ...
  for(const m of customModels){if(destinations.some(d=>d.id===m.id))continue;const d={id:m.id,name:m.label,center:[(m.bounds[0]+m.bounds[2])/2,(m.bounds[1]+m.bounds[3])/2],zoom:19,pitch:55,bearing:0,...m.view};destinations.push(d);$('destination').append(new Option(d.name,d.id));}
  syncDetailedExclusions();
  if(customModels.some(m=>'#'+m.id===location.hash))navigate(location.hash.slice(1),'3d');
 }catch(error){if(!walking)console.error('Detailed models:',error);}
}
function syncDetailedExclusions(){
  const ids=[...customModels.flatMap(model=>model.excludeIds),...mapBombDamage.ids];
  const filter=['all',['!',['in',['get','osm_id'],['literal',ids]]]];
  const intact=['!',['in',['get','osm_id'],['literal',mapBombDamage.ids]]];
  if(JSON.stringify(map.getFilter('buildings-flat'))!==JSON.stringify(intact))map.setFilter('buildings-flat',intact);
...
function syncBuildingVisibility(){
  ...
  if(bombFragments&&(bombFragments.visible!==$('buildings-toggle').checked||bombFragments.photos!==photoMode))bombFragments.setVisible($('buildings-toggle').checked,photoMode);
  const visibility=$('buildings-toggle').checked&&mapBuildingsVisible(map)?'visible':'none';
...
function wireControls(){
  ...
  $('overview').onclick=()=>{map.fitBounds(BOUNDS,{padding:isMobile()?{top:80,bottom:80,left:35,right:35}:{top:80,bottom:90,left:80,right:80},pitch:0,bearing:0,duration:transition()});$('location-label').textContent=CITY.name.toLocaleUpperCase('fr-FR')+' ET ALENTOURS';$('destination').value='';setPanel(false);};
...
async function loadMetadata(){
  const responses=await Promise.all(['manifest.json','places.geojson'].map(async f=>{const r=await fetch(cityDataURL(f));if(!r.ok)throw Error(`Données indisponibles (${r.status})`);return r.json();}));
  ...
  destinations=CITY.id==='nice'?CITY.destinations.map(d=>({...d})):[destinations[0],CAMP127,PARC14,BUIRETTE,BUIRETTE_ODD,BUIRETTE_15,ATTILA,ATTILA_REAR,ATTILA_STREET,ATTILA_TERRACE,ATTILA_CORNER,NERVAL_FOCUS,NERVAL_REAR,NERVAL_GARDEN,NERVAL_END,NERVAL,...wanted.map(name=>places.find(f=>f.properties.name===name)).filter(Boolean).map(f=>({id:f.properties.osm_id,name:f.properties.name,center:f.geometry.coordinates,zoom:15.3}))];
...
function exposeMapTools(){
  ...
  register({name:'read_map_state',description:'Lire le point de vue actuel, le rendu photo et les destinations disponibles de la ville sélectionnée.',inputSchema:{type:'object',properties:{},additionalProperties:false},annotations:{readOnlyHint:true,untrustedContentHint:true},execute:()=>walking?walkMode.getState():({city:CITY.id,center:map.getCenter().toArray(),zoom:map.getZoom(),pitch:map.getPitch(),rendering:photoMode?'photo':'map',roofs:roofTextures?.getState(),roads:cityRoads?.getState(),highwind:mapHighwind?.getState(),buildings3DVisible:mapBuildingsVisible(map),nerval:nervalLayer?.getState(),attila:attilaLayer?.getState(),customModels:Object.fromEntries([...customLayers].map(([id,l])=>[id,l.getState()])),streetViews:nervalUI?.getState(),destinations})});
  if(CITY.id==='chalons')register({name:'visit_nerval',description:'Afficher l’un des six points de vue de la rue Gérard-de-Nerval.',inputSchema:{type:'object',properties:{stop:{type:'integer',minimum:1,maximum:6}},required:['stop'],additionalProperties:false},annotations:{readOnlyHint:false},execute:async input=>{if(!Number.isInteger(input?.stop)||input.stop<1||input.stop>6)throw Error('Point de vue entre 1 et 6 requis.');nervalUI.visit(input.stop-1);if(map.isMoving())await map.once('moveend');return nervalUI.getState();}});
...
async function boot(){
  if(walkEntry||autoPilot){
    walkMode=new WalkMode({map:null,onStart:()=>{walking=true;setPanel(false);}});
    await walkMode.start({entry:walkEntry,pilot:autoPilot});
    return;
  }
  ...
  try{
    map=new maplibregl.Map({container:'map',style:{version:8,glyphs:`${location.origin}${location.pathname.replace(/[^/]*$/,'')}fonts/{fontstack}/{range}.pbf`,sources:{},layers:[{id:'background',type:'background',paint:{'background-color':'#edf0e3'}}],light:{anchor:'viewport',color:'#fff6e8',intensity:0.42,position:[1.3,200,45]}},...(walkReturn??(location.hash==='#camp127'?CAMP127:location.hash==='#parc14'?PARC14:location.hash==='#buirette-15'?BUIRETTE_15:location.hash==='#buirette-17-19'?BUIRETTE_ODD:location.hash==='#buirette'?BUIRETTE:location.hash==='#attila'?ATTILA:location.hash==='#attila-rear'?ATTILA_REAR:location.hash==='#attila-street'?ATTILA_STREET:location.hash==='#attila-terrace'?ATTILA_TERRACE:location.hash==='#nerval-rear'?nervalRearView():location.hash==='#nerval-focus'?nervalFocusView():location.hash==='#nerval'?NERVAL:overviewCamera(CITY))),maxBounds:NAV_BOUNDS,minZoom:10.4,maxZoom:22,maxPitch:80,canvasContextAttributes:{antialias:!isMobile()},pixelRatio:Math.min(devicePixelRatio||1,isMobile()?1.5:2),renderWorldCopies:false,attributionControl:false,locale:{'Popup.Close':'Fermer la fiche'}});
    ...
    let dataError=false;
    map.on('error',e=>{console.error('Map error:',e.error);if(e.sourceId==='ign-photo'||String(e.error?.url||e.error).includes('/imagery/ign/'))return;dataError=true;status('Certaines données n’ont pas pu charger.','error');});
    ...
    await Promise.all([map.once('load'),loadMetadata(),loadTerrain(),loadMapBombDamage(),loadBombMapRoads()]);
    installMapTerrain(map,maplibregl);
    map.setSky({...SKY_STYLE});
    map.addLayer(new MapSkyLayer());
    map.setPadding(cameraPadding());addLayers();mapHighwind=new MapHighwind({pose:readMapHighwind()});if(mapHighwind.enabled)map.addLayer(mapHighwind);initialized=true;setLayer('labels',false);setPhotoMode(photoMode);wireControls();syncCamera();
